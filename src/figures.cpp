...
public:
  virtual void updateGameView(RenderWindow &win) = 0;
  virtual void updateMouse(RenderWindow &win) = 0;
  virtual void gameOver(RenderWindow &win) = 0;
...
public:
  Sources(RenderWindow &win)
      : yAccelerationDown{0}, xVelocity{6}, enemyVelocity{10},
        ...
        isMoving{false}, looksRight{true}, looksLeft{false} {
    ...
    font.loadFromFile("fonts/Repka.ttf");
    ...
    monsterDeathSound.setVolume(15);
...
private:
  ...
  void addItem() override {
    if (!isLost && clockForTiles.getElapsedTime().asSeconds() > 0.1 &&
        player.getPosition().y < scoreInt + 10) {
...
void Sources::updateScore() {
  ...
  if (isLost) {
    ifstream highscoreFileRead("game/highscore.txt");
    string tmp = "", tmp2 = "0";
    ...
    ofstream highscoreFileWrite("game/highscore.txt");
